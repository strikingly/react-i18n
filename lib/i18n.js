...
var _sprintfJsMod = require('./sprintf-js-mod');
...
var LOCALE_DEBUG = false;

var i18n = null;
var _cache = {};

function setLocale(jedInstance) {
  i18n = jedInstance;
}
...
setLocale(new _jed2['default']({
  'domain': DOMAIN,
  'missing_key_callback': function missing_key_callback(key) {},
  'locale_data': _defineProperty({}, DOMAIN, {
    '': {
      'domain': DOMAIN,
      'lang': 'en',
...
function formatForReact(formatString, args) {
  ...
  _sprintfJsMod.sprintf.parse(formatString).forEach(function (match, idx) {
    if (_lodash2['default'].isString(match)) {
      rv.push(match);
    } else {
      var arg = null;
      if (match[2]) {
      ...
      if (_react2['default'].isValidElement(arg)) {
        rv.push(_react2['default'].cloneElement(arg, { key: idx }));
        ...
        // index and then pass an array with two items in.
          ...
          match[2] = null;
          match[1] = 1;
          rv.push(_react2['default'].createElement(
            _reactNative.Text,
            { key: idx++ },
            _sprintfJsMod.sprintf.format([match], [null, arg])
          ));
        }
    }
  });
...
function argsInvolveReact(args) {
  if (args.some(_react2['default'].isValidElement)) {
    return true;
  }
  if (args.length == 1 && _lodash2['default'].isObject(args[0])) {
    return Object.keys(args[0]).some(function (key) {
      return _react2['default'].isValidElement(args[0][key]);
    });
  }
  return false;
}
...
function parseComponentTemplate(string) {
  ...
  function process(startPos, group, inGroup) {
    ...
    while ((match = regex.exec(string)) !== null) {
      ...
      var substr = string.substr(pos, match.index - pos);
      if (substr !== '') {
        buf.push(substr);
      }
...
function renderComponentTemplate(template, components) {
  ...
  function renderGroup(group) {
    ...
    (template[group] || []).forEach(function (item) {
    ...
    var reference = components[group] || _react2['default'].createElement(_reactNative.Text, { key: idx++ });
    if (!_react2['default'].isValidElement(reference)) {
    ...
    if (children.length > 0) {
      return _react2['default'].cloneElement(reference, { key: idx++ }, children);
    } else {
      return _react2['default'].cloneElement(reference, { key: idx++ });
    }
  }
...
function mark(rv) {
  ...
  // 3 cases
  if (_react2['default'].isValidElement(rv)) {
  ...
  } else if (_lodash2['default'].isArray(rv)) {
    // Need to be wrapped
    return [].concat(_toConsumableArray(rv), [' 🇦🇹']);
  } else {
...
function format(formatString, args) {
  if (argsInvolveReact(args)) {
    ...
    return _sprintfJsMod.sprintf.apply(undefined, [formatString].concat(_toConsumableArray(args)));
  }
}

function gettext(string) {
  var rv = cacheGettext(string);
...
var tn = ngettext;
exports.tn = tn;
var tct = gettextComponentTemplate;
exports.tct = tct;